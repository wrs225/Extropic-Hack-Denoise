"""
Parameter Sweep for Peppers Image with Different Salt & Pepper Noise Levels
...
def load_peppers_image(peppers_path):
    """Load and convert peppers image to 4-bit greyscale."""
    img = Image.open(peppers_path)
...
def run_parameter_sweep(noisy_img, original_img, noise_level, output_dir):
    ...
    print(f"\n{'='*70}")
    print(f"PARAMETER SWEEP - PEPPERS IMAGE - SALT & PEPPER NOISE {noise_level}")
    print(f"{'='*70}")
...
def create_heatmap(results, alpha_values, beta_values, noise_level, output_dir):
    ...
    ax.set_title(f'PSNR Improvement (dB) - Peppers - Salt&Pepper Noise={noise_level}', fontsize=14, fontweight='bold')
...
def main():
    """Run parameter sweeps for peppers image with different salt & pepper noise levels."""

    # Load peppers image
    print("Loading peppers image...")
    peppers_path = "/home/will/Extropic-Hack/data/peppers.jpeg"
    original_img = load_peppers_image(peppers_path)
    print(f"Loaded peppers image: {original_img.shape}, range: [{original_img.min()}, {original_img.max()}]")

    # Save original
    os.makedirs("build/peppers_sweeps", exist_ok=True)
    Image.fromarray((original_img * 17).astype(np.uint8)).save("build/peppers_sweeps/original.png")
    ...
    for noise_level in noise_levels:
        ...
        # Save noisy image
        noisy_path = f"build/peppers_sweeps/noisy_{noise_level}.png"
        Image.fromarray((noisy_img * 17).astype(np.uint8)).save(noisy_path)
        print(f"Noisy image saved: {noisy_path}")

        # Run sweep
        output_dir = f"build/peppers_sweeps/noise_{noise_level}"
    ...
    # Summary
    print(f"\n\n{'='*70}")
    print("FINAL SUMMARY - PEPPERS IMAGE")
    print(f"{'='*70}")
    for noise_level in noise_levels:
